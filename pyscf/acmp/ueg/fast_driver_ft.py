...
def get_gp1_v1(occs, nocc, eig_o, nelec0, beta, nbas, rs0, occ_tol,
    ...
    occs = occs[:nocc]
    n0 = occs.sum()
    hcore = numpy.diag(tmp_ueg.get_hcore())
    veff = numpy.diag(tmp_ueg.get_veff())
    v1 = (hcore - eig_o)
...
def get_gp2(STYLE, method, nelec0, beta, rs0, nbas, occs, nocc, nvir,
    ...
    if STYLE == "ac":
        ...
        my_ueg.occs = occs
        my_ueg.nocc = nocc
        eigk = 0.5 * numpy.diag(my_ueg.get_veff())
...
def run_ueg_calc(**settings):
    ...
    e0 = get_e0(my_ueg.occs, my_ueg.nocc, eig_o)
    gp1, v1 = get_gp1_v1(my_ueg.occs, nocc, eig_o,
    ...
    if ecorr_method == "finite_difference":
        ...
        for _beta in [beta + 0.5 * dbeta, beta - 0.5 * dbeta]:
            _occs = _fermi_smearing_occ(my_ueg.mu, eig_o, 1.0 / _beta)
            _gp1, _v1 = get_gp1_v1(_occs, nocc, eig_o, nelec, _beta,
                                   nbas, rs, occ_tol, apply_ks_dv)
...
def post_process_ac(my_ueg, method, w0, w0p):
    aci = method["aci"]
    df_codes = method["df_codes"] + [method["silim"]]
    w_list = [w0p.copy(), w0.copy()]
    rhovec = my_ueg.mgga_rho_vector()
